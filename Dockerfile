...
# Build application
COPY . .
RUN cargo build --release --target x86_64-unknown-linux-musl --bin rtsp-recorder
...
WORKDIR /app
COPY --from=builder /app/target/x86_64-unknown-linux-musl/release/rtsp-recorder /app/rtsp-recorder
ENTRYPOINT ["/app/rtsp-recorder"]
